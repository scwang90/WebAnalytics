...
//    @Select("SELECT id , idsite , create_time createTime , update_time updateTime , new_user newUser , idvisitor , visit_servertime visitServertime , visit_localtime visitLocaltime , visit_totaltime visitTotaltime , idurl , idtitle , idurl_entry idurlEntry , idtitle_entry idtitleEntry , idurl_exit idurlExit , idtitle_exit idtitleExit , url_referer urlReferer , useragent , operate_system operateSystem , operate_version operateVersion , browser_name browserName , browser_version browserVersion , app_name appName , screen_resolution screenResolution , screen_depth screenDepth , location_ip locationIp , location_lang locationLang , location_country locationCountry , location_region locationRegion , location_city locationCity , end_model endModel , end_brand endBrand , end_type endType , net_type netType , count_visits countVisits , count_events countEvents FROM t_visit WHERE  idsite=#{siteId} AND idvisitor=#{idvtor}")
//    Visit getVisit(@Param("siteId") int siteId,@Param("idvtor")  String idvtor) throws Exception;

    @Select("SELECT id , idsite , idsubsite , create_time createTime , update_time updateTime , new_user newUser , idvisitor , visit_servertime visitServertime , visit_localtime visitLocaltime , visit_totaltime visitTotaltime , idurl , idtitle , idurl_entry idurlEntry , idtitle_entry idtitleEntry , idurl_exit idurlExit , idtitle_exit idtitleExit , url_referer urlReferer , useragent , operate_system operateSystem , operate_version operateVersion , browser_name browserName , browser_version browserVersion , app_name appName , screen_resolution screenResolution , screen_depth screenDepth , location_ip locationIp , location_lang locationLang , location_country locationCountry , location_region locationRegion , location_city locationCity , end_model endModel , end_brand endBrand , end_type endType , net_type netType , count_visits countVisits , count_events countEvents FROM t_visit WHERE  idsite=#{siteId} AND idvisitor=#{idvtor} AND visit_servertime > DATE_SUB(NOW(),INTERVAL 30 MINUTE) ")
    Visit getVisitHalfHour(@Param("siteId") int siteId,@Param("idvtor")  String idvtor) throws Exception;
    ...
    void insertUrl(Url url) throws Exception;
    ...
    void insertAction(Action action) throws Exception;
    /**
     * 添加事件
     * @param event event
     */
