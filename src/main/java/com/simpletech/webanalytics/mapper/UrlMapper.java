...
public interface UrlMapper extends MybatisMultiDao<Url>{
	 ...
	 * @return 改变的行数
	 */
	@Insert("INSERT INTO t_url ( id , idsite , create_time , update_time , hash , url ) VALUES ( #{id} , #{idsite} , #{createTime} , #{updateTime} , #{hash} , #{url} )")
	 ...
	 * @return 改变的行数
	 */
	@Update("UPDATE t_url SET id=#{id} , idsite=#{idsite} , create_time=#{createTime} , update_time=#{updateTime} , hash=#{hash} , url=#{url} WHERE id=#{id} ")
	 ...
	 */
	@Select("SELECT id , idsite , create_time createTime , update_time updateTime , hash , url FROM t_url WHERE id=#{id}")
	Url findById(@Param("id") Object id) throws Exception;
	/**
	 * 获取全部数据
	 * @return 全部数据列表
	 */
	@Select("SELECT id , idsite , create_time createTime , update_time updateTime , hash , url FROM t_url ${order}")
	 ...
	 * @return 分页列表数据
	 */
	@Select("SELECT id , idsite , create_time createTime , update_time updateTime , hash , url FROM t_url ${order} LIMIT ${start},${limit}")
	 ...
	 * @return 符合条件的列表数据
	 */
	@Select("SELECT id , idsite , create_time createTime , update_time updateTime , hash , url FROM t_url ${where} ${order} LIMIT ${start},${limit}")
	 ...
	 * @return 返回符合条件的数据列表
	 */
	@Select("SELECT id , idsite , create_time createTime , update_time updateTime , hash , url FROM t_url WHERE ${propertyName}=#{value} ${order}")
