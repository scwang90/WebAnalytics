 ...
 * @author 树朾
 * @date 2015-09-25 15:38:38 中国标准时间
 */
public interface ActionMapper extends MybatisMultiDao<Action>{
	 ...
	 * @return 改变的行数
	 */
	@Insert("INSERT INTO t_action ( id , create_time , update_time , idsite , idvisitor , idvisit , idtitle , idurl , server_time , time_spent ) VALUES ( #{id} , #{createTime} , #{updateTime} , #{idsite} , #{idvisitor} , #{idvisit} , #{idtitle} , #{idurl} , #{serverTime} , #{timeSpent} )")
	 ...
	 * @return 改变的行数
	 */
	@Update("UPDATE t_action SET id=#{id} , create_time=#{createTime} , update_time=#{updateTime} , idsite=#{idsite} , idvisitor=#{idvisitor} , idvisit=#{idvisit} , idtitle=#{idtitle} , idurl=#{idurl} , server_time=#{serverTime} , time_spent=#{timeSpent} WHERE id=#{id} ")
	 ...
	 */
	@Select("SELECT id , create_time createTime , update_time updateTime , idsite , idvisitor , idvisit , idtitle , idurl , server_time serverTime , time_spent timeSpent FROM t_action WHERE id=#{id}")
	Action findById(@Param("id") Object id) throws Exception;
	/**
	 * 获取全部数据
	 * @return 全部数据列表
	 */
	@Select("SELECT id , create_time createTime , update_time updateTime , idsite , idvisitor , idvisit , idtitle , idurl , server_time serverTime , time_spent timeSpent FROM t_action ${order}")
	 ...
	 * @return 分页列表数据
	 */
	@Select("SELECT id , create_time createTime , update_time updateTime , idsite , idvisitor , idvisit , idtitle , idurl , server_time serverTime , time_spent timeSpent FROM t_action ${order} LIMIT ${start},${limit}")
	 ...
	 * @return 符合条件的列表数据
	 */
	@Select("SELECT id , create_time createTime , update_time updateTime , idsite , idvisitor , idvisit , idtitle , idurl , server_time serverTime , time_spent timeSpent FROM t_action ${where} ${order}")
	 ...
	 * @return 符合条件的列表数据
	 */
	@Select("SELECT id , create_time createTime , update_time updateTime , idsite , idvisitor , idvisit , idtitle , idurl , server_time serverTime , time_spent timeSpent FROM t_action ${where} ${order} LIMIT ${start},${limit}")
	 ...
	 * @return 返回符合条件的数据列表
	 */
	@Select("SELECT id , create_time createTime , update_time updateTime , idsite , idvisitor , idvisit , idtitle , idurl , server_time serverTime , time_spent timeSpent FROM t_action WHERE ${propertyName}=#{value} ${order}")
