...
public interface SharePointMapper extends MybatisMultiDao<SharePoint>{
	 ...
	 * @return 改变的行数
	 */
	@Delete("DELETE FROM t_share_point WHERE id=#{id}")
	 ...
	 * @return 统计数
	 */
	@Select("SELECT COUNT(*) FROM t_share_point")
	 ...
	 * @return 改变的行数
	 */
	@Delete("DELETE FROM t_share_point ${where}")
	 ...
	 * @return 改变的行数
	 */
	@Delete("DELETE FROM t_share_point WHERE ${propertyName}=#{value}")
	 ...
	 * @return 统计数
	 */
	@Select("SELECT COUNT(*) FROM t_share_point ${where}")
